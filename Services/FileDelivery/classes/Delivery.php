...
use ILIAS\HTTP\Status\StatusInterface as Status;
...
class Delivery {
	...
	public function deliver() {
		...
		$this->headers()->add('X-ILIAS-FileDelivery-Method', $this->getDeliveryType());
		$this->cleanDownloadFileName();
		$this->clearBuffer();
		$this->checkCache();
		$this->setGeneralHeaders();
		$this->delivery()->prepare($this->getPathToFile(), $this->headers());
		$this->headers()->sendAll();
		$this->delivery()->deliver($this->getPathToFile(), $this->headers());
	...
	public function setGeneralHeaders() {
		...
		if ($this->isSendMimeType()) {
			$this->headers()->add(Headers::CONTENT_TYPE, $this->getMimeType());
		}
		...
		$this->headers()->add(Headers::ACCEPT_RANGES, 'bytes');
		if ($this->getDeliveryType() == DeliveryType::DELIVERY_METHOD_PHP && $this->getPathToFile() != self::DIRECT_PHP_OUTPUT) {
			$this->headers()->add(Headers::CONTENT_LENGTH, (string)filesize($this->getPathToFile()));
		}
		$this->headers()->add(Headers::CONNECTION, "close");
	}


	public function setCachingHeaders() {
		$this->headers()->add(Headers::CACHE_CONTROL, 'must-revalidate, post-check=0, pre-check=0');
		$this->headers()->add(Headers::PRAGMA, 'public');
		$this->sendEtagHeader();
	...
	protected function sendEtagHeader() {
		if ($this->getEtag()) {
			$this->headers()->add(Headers::E_TAG, $this->getEtag() . '');
		}
	}
	...
	protected function sendLastModified() {
		if ($this->getShowLastModified()) {
			$this->headers()->add(Headers::LAST_MODIFIED, date("D, j M Y H:i:s", filemtime($this->getPathToFile())) . " GMT");
		}
	}
	...
	protected function setDispositionHeaders() {
		$this->headers()->add(Headers::CONTENT_DISPOSITION, $this->getDisposition() . '; filename="' . $this->getDownloadFileName() . '"');
		$this->headers()->add(Headers::CONTENT_DESCRIPTION, $this->getDownloadFileName());
	}
}
